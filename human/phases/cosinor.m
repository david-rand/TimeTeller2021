function [p_3a,phase, maxf ,antiphase ,minf] = cosinor(t,y,w,alpha)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Input:
%   t - time series
%   y - value of series17.4400   & 0.51 at time t
...
maxf = max(f);
minf = min(f);
phases = (find(f==max(f))-1)/100;
phases = phases*24+4; %% why plus 4???
phase=phases(1);
antiphases = (find(f==min(f))-1)/100;
antiphases = antiphases*24+4; %why plus 4??
antiphase=antiphases(1);
...
%         xlim([min(t) max(t)]);
% %          xticks(linspace(0,1,7))
% %  set(gca,'XTickLabel',[times,'4am'])
% %  text(phase,7,phase*24)
%      % % Display results
